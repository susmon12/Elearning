  ...
  const handlepayment = async () => {
    try {
      if (data && data.email) {
        ...
        const res = await payment(actualData);
        handleCourseActivation();
        setPaymentExecuted(true); // Set the state to true after the payment is executed
      } else {
        console.error('User data not available.');
      }
    } catch (error) {
            ...
            <div className="headerpay">
              ...
              <div className="actionspay">
                <Link to="/profile">
                  ...
                  {!paymentExecuted && (
                    <button ref={buttonRef} className="history" id='cltry' onClick={handlepayment} style={{ display: "none" }} type="button"></button>
                  )}
              </div>
            </div>
          </div>
